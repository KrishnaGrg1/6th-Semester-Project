...
function displayTrendingMovie(movie) {
    ...
    fetchMovieTrailer(movie.id).then(trailerUrl => {
        if (trailerUrl) {
           ...
           watchBtn.parentElement.removeChild(watchBtn);
...
async function displayMovieTrailers(movies) {
    ...
    for (let movie of movies) {
        ...
        if (trailerUrl) {
            ...
            // If no trailer is found, display the poster instead
            const posterImg = document.createElement('img');
            posterImg.src = posterPath;
            posterImg.alt = title;
            posterImg.classList.add('poster-image'); // Optional: add a class for styling
            
            // Add poster image to the trailer item
            trailerItem.appendChild(posterImg);
            
           
        }
        
        // Append the trailer item to the trailers container
